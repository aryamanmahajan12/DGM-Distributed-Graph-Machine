...
int main(int argc,char** argv){
    ...
    while(!done){
        ...
        if(id == 0){
            ...
            // parse broadcast "flag;n;parent0,parent1,..."
            if(rec.rfind("MST;",0) == 0){
                // Rare: if root published MST before this worker subscribed in time. But typical flow: we always receive done broadcast first.
                // If happens, handle MST message below by printing then continue.
            }
                ...
                if(p1==string::npos || p2==string::npos){
                    ...
                    if(done_flag) {
                        ...
                        if(mstrec.rfind("MST;",0)==0){
                            ...
                            int mcount = stoi(mstrec.substr(p+1, q-(p+1)));
                            string body = mstrec.substr(q+1);
                            if(mcount==0){
